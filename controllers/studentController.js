...
const headCount = async () => {
  try {
    const result = await Student
      ...
      console.error('There are no students.', err);
      throw err;
  }
}
...
const grade = async (studentId) => {
  // Convert the studentId string into an ObjectId
  const studentObjectId = new ObjectId(studentId);
    // TODO: Ensure we include only the student who can match the given ObjectId using the $match operator
    // Execute the aggregate method on the Student model
  const result = await Student.aggregate([
    // Match the documents based on the student's ID
    { $match: { _id: studentObjectId } },
    // Deconstruct the assignments array
    { $unwind: '$assignments' },
    // Group the documents by student and calculate the average grade
    {
      $group: {
        _id: '$_id',
